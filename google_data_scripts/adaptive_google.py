...
_,_,_,_,feat_google_step3,sav_path,_ = get_paths()
...
#%%
dat_gg_obi = os.path.join(feat_google_step3,'XY_test_ready.obj')

df_test_xy = loadDatasetObj(dat_gg_obi)['XY_test_ready']
id_m = 'M_id'
X_test = scaler.transform(np.array(df_test_xy.drop(['M_id','y'],axis=1)))
y_test  = np.array(df_test_xy['y'])
#%%.
save_pred_save = sav_path
