...
class LSTMHyperparameterOptimization(Problem):
    def __init__(self, num_feat,num_epoc,batch_size,save_path,model_name):
    ...
    def _evaluate(self, x):
        ...
        if flag_ali == 1:
            model.export(os.path.join(self.save_path,'Alibaba'))
        ...
        if flag_BB == 1:
            model.export(os.path.join(self.save_path,'BB'))
        ...
        if flag_google == 1:
            model.export(os.path.join(self.save_path,'Google'))
...
#%%
if run_search: 
    problem = LSTMHyperparameterOptimization(num_feat,num_epoc,batch_size,save_path,model_name)
