...
for counter,file_i in enumerate(arr):
    ...
    train_len = int(len(df)*(1-test_per))
    if counter == 0:
        X_train = df["X"].loc[:train_len]
        y_train = df["y"][:train_len]
        
        X_test = df["X"].loc[train_len:]
        y_test = df["y"][train_len:]
    else:
        X_train  = pd.concat([X_train, df["X"].loc[:train_len]], ignore_index=True)
        y_train = y_train + df["y"][:train_len]
        
        X_test  = pd.concat([X_test, df["X"].loc[train_len:]], ignore_index=True)
