...
data_path = feat_BB_step2
sav_path = feat_BB_step3
...
cpu_name = 'CPU usage [%]'
col_feat_google = [cpu_name+ feat.split(' used percent of cpus(%)')[1] for feat in col_feat_ali if ' used percent of cpus(%)' in feat]
col_feat_google = col_feat_google + ['M_id','y']
#%%
filename = os.path.join(data_path,'X_Y_BB_before_removing_features.obj')
df = loadDatasetObj(filename)
# feat_all = set(list(df.columns))

#%%
data_set = {'XY_test_ready':df['XY_BB'][col_feat_google].dropna(axis=0)}
